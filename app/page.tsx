...
// ── Demo drawer (slides in from the right) ───────────────────────────────────
function DemoDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      {/* Drawer panel */}
      <div className={`fixed inset-y-0 right-0 z-[70] w-full max-w-[480px] bg-stone-950 border-l border-stone-800 shadow-2xl flex flex-col transition-transform duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] ${open ? "translate-x-0" : "translate-x-full"}`}>

        {/* Header */}
        <div className="flex items-start justify-between px-6 py-5 border-b border-stone-800 shrink-0">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <div className="w-5 h-5 rounded bg-emerald-500 flex items-center justify-center">
                <span className="text-white font-bold text-xs leading-none">P</span>
              </div>
              <span className="text-xs font-medium text-emerald-400">Prime Accountax</span>
            </div>
            <h2 className="text-lg font-bold text-white">Request a demo</h2>
            <p className="text-xs text-stone-500 mt-0.5">We'll be in touch within one business day.</p>
          </div>
          <button onClick={onClose}
            className="p-2 rounded-lg text-stone-500 hover:text-white hover:bg-stone-800 transition-colors mt-0.5">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
        ...
        {/* Value props strip */}
        <div className="px-6 py-4 border-b border-stone-800/60 shrink-0">
          <div className="grid grid-cols-2 gap-2">
            {[
              "QuickBooks & Xero sync",
              "Automated collections",
              "Customer payment portal",
              "Bookkeeper-led accounting",
            ].map(item => (
...
export default function LandingPage() {
  ...
  // Open the demo drawer when arriving from a secondary page (e.g. /?demo=1)
  useEffect(() => {
    const p = new URLSearchParams(window.location.search);
    if (p.get("demo") === "1" || window.location.hash === "#demo") setDemoOpen(true);
  }, []);
  ...
  return (
    <div className="relative bg-stone-950 text-stone-100 min-h-screen font-sans antialiased overflow-x-hidden">
        ...
        <div className="max-w-6xl mx-auto px-5 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            <button
              ...
              className="group text-sm font-medium bg-emerald-500 hover:bg-emerald-400 text-white px-4 py-2 rounded-lg transition-all hover:shadow-lg hover:shadow-emerald-500/30 active:scale-95">
              Request a demo →
            </button>
          </div>
          ...
          <div className="md:hidden border-t border-stone-800 bg-stone-950/95 backdrop-blur-xl px-5 pb-4 space-y-1">
            ...
            <div className="pt-3 flex flex-col gap-2">
              ...
              <button onClick={() => { setMenuOpen(false); setDemoOpen(true); }}
                className="text-sm text-center font-medium bg-emerald-500 hover:bg-emerald-400 text-white px-4 py-2.5 rounded-lg transition-colors">
                Request a demo →
              </button>
            </div>
          </div>
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          ...
          <Reveal delay={240}>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
              ...
              <Link href="/login"
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 text-stone-400 hover:text-white text-sm px-4 py-3.5 rounded-xl transition-all">
                Sign in
              </Link>
            </div>
          </Reveal>
      ...
      {/* ── DEMO CTA BAND ── */}
      <section id="contact" className="relative py-16 px-5 border-y border-stone-800/60">
        <div className="max-w-5xl mx-auto">
          <Reveal>
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
              <div>
                <h2 className="text-xl md:text-2xl font-bold text-white mb-1">See Prime Accountax in action</h2>
                <p className="text-stone-400 text-sm max-w-md">
                  Book a personalised demo — we'll walk through your AR workflow, including bookkeeper-led accounting services.
                </p>
              </div>
              <div className="flex items-center gap-3 shrink-0">
                <button
                  onClick={() => setDemoOpen(true)}
                  className="inline-flex items-center gap-2 bg-emerald-500 hover:bg-emerald-400 text-white font-semibold text-sm px-6 py-3 rounded-xl transition-all hover:shadow-xl hover:shadow-emerald-500/30 hover:-translate-y-0.5 active:scale-[0.98]">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" />
                  </svg>
                  Request a demo
                ...
                <Link href="/login"
                  className="inline-flex items-center gap-2 text-stone-300 hover:text-white border border-stone-700 hover:border-stone-500 text-sm px-6 py-3 rounded-xl transition-all">
                  Sign in
                </Link>
              </div>
            </div>
          ...
          <Reveal>
            <div className="relative overflow-hidden rounded-3xl border border-emerald-500/25 bg-gradient-to-b from-emerald-500/10 via-stone-900/60 to-transparent p-10 md:p-14 text-center backdrop-blur-xl shadow-2xl shadow-emerald-900/30">
              ...
              <p className="text-stone-400 mb-8 text-lg max-w-xl mx-auto">
                Book a demo and we'll get your QuickBooks or Xero connected, then manage your receivables from one collection dashboard.
              </p>
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                <button
                  onClick={() => setDemoOpen(true)}
      ...
      <footer className="relative border-t border-stone-800 py-8 px-5">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-stone-500">
          ...
          <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2">
            <Link href="/accounts-receivable-software-for-quickbooks" className="hover:text-emerald-400 transition-colors">AR for QuickBooks</Link>
            <Link href="/accounts-receivable-software-for-xero" className="hover:text-emerald-400 transition-colors">AR for Xero</Link>
            <Link href="/credit-control-software" className="hover:text-emerald-400 transition-colors">Credit control</Link>
