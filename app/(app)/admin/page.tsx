...
import { Building2, Users, Plus, Check, X, Eye, EyeOff, Shield, RefreshCw, Pencil, Loader2, Trash2, Search, AlertTriangle, CreditCard, XCircle, FileText, Clock, CheckCircle2, ChevronRight, LayoutDashboard, ScrollText, Settings, ArrowUpRight, Globe, Zap } from "lucide-react";
...
// ============================================================
function BillingSystemLinks() {
  const ADMIN_PAGES = [
    {
      href:    "/admin/billing",
      icon:    LayoutDashboard,
      label:   "Billing Overview",
      desc:    "Subscription stats, recent cancellations & leads, audit feed",
      color:   "emerald",
    },
    {
      ...
      desc:    "Review pending requests and set Stripe cancellation schedule",
      color:   "amber",
    },
    {
      href:    "/admin/leads",
      icon:    FileText,
      label:   "Leads",
      desc:    "Landing page enquiries and demo requests with status workflow",
      color:   "blue",
    },
    {
      ...
      icon:    CreditCard,
      label:   "Subscriptions",
      desc:    "All Stripe-synced subscriptions across every organisation",
      color:   "stone",
    },
    {
      href:    "/admin/audit",
      icon:    ScrollText,
      label:   "Audit Log",
      desc:    "Immutable record of all billing and subscription events",
      color:   "stone",
    },
  ];
  ...
  const CUSTOMER_PAGES = [
    {
      ...
      desc:    "Plan card, status, payment method, renewal — cancel or reactivate",
      tag:     "Customer-facing",
      color:   "stone",
    },
  ];

  const API_ROUTES: { method: "GET" | "POST"; path: string; desc: string }[] = [
    { method: "GET",  path: "/api/billing",               desc: "Returns subscription + cancellation state for the current org" },
    { method: "POST", path: "/api/billing/cancel-request", desc: "Creates a pending admin-review request — no auto-cancel" },
    { method: "POST", path: "/api/billing/reactivate",    desc: "Removes scheduled cancellation from Stripe + local state" },
    { method: "POST", path: "/api/billing/portal",        desc: "Opens Stripe Customer Portal for payment method updates" },
  ];
  ...
  return (
    <div className="space-y-4 pt-2">
      <div className="flex items-center gap-2">
        ...
        <p className="text-[11px] font-semibold text-stone-500 uppercase tracking-widest px-2">Billing System</p>
        <div className="h-px flex-1 bg-stone-800" />
      </div>

      {/* Admin pages */}
      <div>
        <p className="text-[11px] text-stone-600 uppercase tracking-wider font-semibold mb-2 flex items-center gap-1.5">
          <Shield size={10} /> Admin pages
        </p>
        <div className="grid sm:grid-cols-3 gap-2">
          {ADMIN_PAGES.map(p => (
            <Link key={p.href} href={p.href}
              className="group flex items-start gap-3 p-3 rounded-xl border border-stone-800 bg-stone-900 hover:border-stone-600 hover:bg-stone-800/60 transition-all">
              <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${colorIcon[p.color]}`}>
                <p.icon size={13} />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1">
                  <span className="text-xs font-semibold text-stone-200 group-hover:text-white transition-colors">{p.label}</span>
                  <ArrowUpRight size={10} className="text-stone-600 group-hover:text-stone-400 transition-colors" />
                </div>
          ...
          {CUSTOMER_PAGES.map(p => (
            <Link key={p.href} href={p.href}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-semibold text-stone-200 group-hover:text-white transition-colors">{p.label}</span>
                  <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-blue-500/15 text-blue-400 font-medium">{p.tag}</span>
                  <ArrowUpRight size={10} className="text-stone-600 group-hover:text-stone-400 transition-colors ml-auto" />
                </div>
                <p className="text-[11px] text-stone-500 mt-0.5 leading-snug">{p.desc}</p>
              </div>
            </Link>
      ...
      <div>
        <p className="text-[11px] text-stone-600 uppercase tracking-wider font-semibold mb-2 flex items-center gap-1.5">
          <Zap size={10} /> API endpoints
        </p>
        <div className="rounded-xl border border-stone-800 bg-stone-900 divide-y divide-stone-800/70 overflow-hidden">
          {API_ROUTES.map(r => (
            <div key={r.path} className="flex items-center gap-3 px-4 py-2.5">
              ...
              <span className="text-xs font-mono text-stone-300 shrink-0">{r.path}</span>
              <span className="text-[11px] text-stone-500 truncate ml-auto hidden sm:block">{r.desc}</span>
            </div>
          ))}
        </div>
      </div>
