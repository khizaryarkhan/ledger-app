...
type Account = {
  ...
  ownerAdminId: string | null; ownerName: string | null; createdAt: string | null; lastActivity: number | null;
};
...
type SortKey = "ref" | "name" | "email" | "stage" | "status" | "planName" | "billing" | "source" | "mrr" | "deals" | "renewsAt" | "owner" | "createdAt" | "lastActivity";
...
export default function AccountsPage() {
  ...
  const load = useCallback(() => {
    ...
    Promise.all([
      ...
      setAccounts(a.accounts ?? []); setAdmins(a.admins ?? []); setNeedsSetup(!!a.needsSetup);
  ...
  const assignOwner = async (accountId: string, ownerAdminId: string) => {
    const name = admins.find(x => x.id === ownerAdminId)?.name ?? admins.find(x => x.id === ownerAdminId)?.email ?? null;
    setAccounts(prev => prev.map(a => a.id === accountId ? { ...a, ownerAdminId: ownerAdminId || null, ownerName: ownerAdminId ? name : null } : a));
    try {
      await fetch(`/api/admin/accounts/${accountId}`, {
        method: "PATCH", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ownerAdminId: ownerAdminId || null }),
      });
    } catch { load(); }
  };
              ...
              {rows.length === 0 ? (
                ...
                return (
                  <tr key={c.accountId} onClick={() => open(c)} className="border-b border-stone-800/50 hover:bg-stone-800/30 cursor-pointer">
                    ...
                    <td className="px-3 py-2.5 whitespace-nowrap" onClick={e => e.stopPropagation()}>
                      {isSuperAdmin ? (
                        <select value={c.ownerId ?? ""} onChange={e => assignOwner(c.accountId, e.target.value)}
                          className="h-7 px-1.5 text-[11px] rounded bg-stone-800/60 border border-stone-700 text-stone-300 focus:border-stone-500 focus:outline-none max-w-[140px]">
                          <option value="">Unassigned</option>
                          {admins.map(ad => <option key={ad.id} value={ad.id}>{ad.name || ad.email}</option>)}
                        </select>
                      ) : (
                        <span className="text-[12px] text-stone-400">{c.owner ?? "—"}</span>
                      )}
                    </td>
                    <td className="px-3 py-2.5 text-[11px] text-stone-400 whitespace-nowrap">{fmtDate(c.createdAt)}</td>
                    <td className="px-3 py-2.5 text-[11px] text-stone-400 whitespace-nowrap">{fmtDate(c.lastActivity)}</td>
