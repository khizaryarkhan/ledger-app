import { db } from "@/db";
import { customers, reps, regions, countries } from "@/db/schema";
...
export async function POST(req: Request) {
  ...
  const { ids, repId, regionId, countryId } = await req.json();
  ...
  if (countryId && !(await ownsInOrg(countries, countryId, orgId!))) return bad("Country not found in this organisation", 404);

  const patch: Record<string, any> = {};
  if (repId !== undefined) patch.repId = repId || null;
  if (regionId !== undefined) patch.regionId = regionId || null;
  if (countryId !== undefined) patch.countryId = countryId || null;
