...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const htmlBody = `
    <div style="font-family:sans-serif;color:#1c1917;max-width:600px;margin:0 auto">
      ...
      <div style="background:#fafaf9;border:1px solid #e7e5e4;border-top:none;padding:24px 28px;border-radius:0 0 8px 8px">
        ...
        ${message ? `<p style="font-size:14px;margin:0 0 16px;white-space:pre-wrap">${message.replace(/</g,"&lt;").replace(/>/g,"&gt;")}</p>` : ""}
        <table style="width:100%;border-collapse:collapse;margin-bottom:20px;font-size:13px">
          ...
          ${bill.dueDate ? `<tr><td style="padding:6px 0;color:#78716c">Due Date</td><td style="padding:6px 0">${new Date(bill.dueDate + "T00:00:00").toLocaleDateString("en-GB",{day:"2-digit",month:"short",year:"numeric"})}</td></tr>` : ""}
          <tr><td style="padding:6px 0;color:#78716c">Amount</td><td style="padding:6px 0;font-weight:600;font-size:16px">${bill.currency === "GBP" ? "£" : bill.currency === "EUR" ? "€" : "$"}${bill.total.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}</td></tr>
        </table>
  ...
  try {
    await sendEmail(orgId!, {
      to: approverEmail,
      subject: `[Action Required] Approve ${billLabel} — ${orgName}`,
      html: htmlBody,
      text: `Please review and approve ${billLabel}.\n\nApproval link: ${portalUrl}\n\nThis link expires in 7 days.`,
    });
  } catch (e: any) {
