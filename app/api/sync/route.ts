import { db } from "@/db";
import { qboTokens, xeroTokens, qboSyncLog, xeroSyncLog } from "@/db/schema";
...
export async function POST() {
  ...
  if (!qboToken && !xeroToken) {
    return bad("No accounting integration connected. Connect QuickBooks or Xero in Settings → Integrations.", 400);
  }
  ...
  const hasError = result.qbo?.error || result.xero?.error;
