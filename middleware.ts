...
export default auth((req) => {
  ...
  const isPublic = isHome || path === "/login" || path === "/forgot-password" || path === "/reset-password" || path.startsWith("/api/auth") || path === "/api/qbo/callback" || path === "/api/gmail/callback" || path === "/api/microsoft/callback" || path === "/api/debug-auth" || isPortal || isLegal;
