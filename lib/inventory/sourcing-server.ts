...
export async function findSourcingViolations(
  ...
  const items = await db.select({ id: apItems.id, name: apItems.name, sourcingPolicy: apItems.sourcingPolicy, productType: apItems.productType, canBePurchased: apItems.canBePurchased })
